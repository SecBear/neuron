...
use skg_op_compute_runtime::{
    ComputeConfig, ComputeError, ComputeRuntime, ComputeSession, ExecutionProfile, ExecutionReport,
};
...
impl ComputeRuntime for EchoRuntime {
    ...
    async fn inspect(&self, session_id: &SessionId) -> Result<ComputeSession, ComputeError> {
        Ok(ComputeSession::new(session_id.clone(), "python"))
    }
}
...
struct CaptureRuntime {
    seen_profile: Mutex<Option<ExecutionProfile>>,
}

#[async_trait]
impl ComputeRuntime for CaptureRuntime {
    async fn exec(
        &self,
        _session_id: &SessionId,
        _code: &str,
        ...
        self.seen_profile.lock().unwrap().replace(profile.clone());
...
#[tokio::test]
async fn compute_operator_uses_configured_profile() {
    let provider = TestProvider::with_responses(vec![make_text_response("```python\nprint('ok')\n```")]);
    let runtime = Arc::new(CaptureRuntime::default());
    let expected_dir = PathBuf::from("/tmp/skelegent-compute-tests");
    let config = ComputeConfig {
        profile: ExecutionProfile {
            working_dir: Some(expected_dir.clone()),
            ..ExecutionProfile::default()
        },
    };
    let op = skg_op_compute_runtime::operator::ComputeOperator::new(provider, Arc::clone(&runtime))
        .with_config(config);

    let _ = op.execute(simple_input("question"), &test_ctx()).await.unwrap();
    let seen = runtime
        .seen_profile
        .lock()
        .unwrap()
        .clone()
        .expect("profile captured");
    assert_eq!(seen.working_dir, Some(expected_dir));
}
