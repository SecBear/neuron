...
use agent_types::{CompletionRequest, ContentBlock, Message, Provider, ProviderError, Role};
...
async fn complete_parses_tool_use_response() {
    ...
    assert_eq!(resp.stop_reason, agent_types::StopReason::ToolUse);
    assert!(matches!(
...
async fn stream_returns_text_deltas() {
    ...
    let events: Vec<agent_types::StreamEvent> = handle.receiver.collect().await;

    // Should have TextDelta events
    let text_deltas: Vec<&str> = events
        .iter()
        .filter_map(|e| {
            if let agent_types::StreamEvent::TextDelta(t) = e {
                Some(t.as_str())
            } else {
                None
            }
    ...
    let has_usage = events.iter().any(|e| {
        matches!(e, agent_types::StreamEvent::Usage(u) if u.input_tokens == 20 && u.output_tokens == 10)
    });
    ...
    let has_complete = events
        .iter()
        .any(|e| matches!(e, agent_types::StreamEvent::MessageComplete(_)));
