//! Tests for the #[agent_tool] derive macro.

use agent_tool::agent_tool;
use agent_types::*;
...
#[agent_tool(name = "echo", description = "Echo text back")]
async fn echo(
    /// The text to echo
    text: String,
    _ctx: &ToolContext,
...
#[agent_tool(name = "add", description = "Add two numbers")]
async fn add(
    /// First number
    a: f64,
    /// Second number
...
async fn derive_tool_registers_in_registry() {
    let mut registry = agent_tool::ToolRegistry::new();
