...
Agentic while loop for rust-agent-blocks. Composes Provider + Tool + Context.
...
## Architecture
- Depends on `agent-types` (traits) and `agent-tool` (ToolRegistry)
- `agent-context` is a dev-dependency only (for tests)
...
- `thiserror` errors come from agent-types (LoopError)
