//! Request/response mapping between agent-types and the OpenAI Chat Completions API format.
//!
...
fn map_image_block(source: &agent_types::ImageSource) -> serde_json::Value {
    match source {
        agent_types::ImageSource::Base64 { media_type, data } => {
            serde_json::json!({
                "type": "image_url",
                "image_url": {
