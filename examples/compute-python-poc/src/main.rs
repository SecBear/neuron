...
```"#,
    ...
    let input = OperatorInput::new(Content::text("Compute the answer"), layer0::operator::TriggerType::User)
                        ...
                        Content::Blocks(blocks) => {
                            for b in blocks {
                                if let layer0::content::ContentBlock::Data { data, .. } = b {
                                    println!("final_result: {}", serde_json::to_string_pretty(&data).unwrap());
                                }
                            }
                        }
                        _ => {}
