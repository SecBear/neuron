...
use agent_types::ContextStrategy;
...
impl Provider for MockProvider {
    ...
    ) -> Result<CompletionResponse, agent_types::ProviderError> {
        let response = self
            .responses
            .lock()
    ...
    async fn complete_stream(
        &self,
        _request: CompletionRequest,
    ) -> Result<agent_types::StreamHandle, agent_types::ProviderError> {
        Err(agent_types::ProviderError::Other(
...
async fn minimal_agent_text_response() {
    ...
    let config = LoopConfig {
        system_prompt: agent_types::SystemPrompt::Text("You are a helpful assistant.".into()),
        ..Default::default()
    };
...
async fn agent_with_tool_calls() {
    ...
    let config = LoopConfig {
        system_prompt: agent_types::SystemPrompt::Text("You can echo text.".into()),
        ..Default::default()
    };
...
async fn agent_with_middleware() {
    ...
    let mut tools = ToolRegistry::new();
    tools.register(EchoTool);
    tools.add_middleware(agent_blocks::tool::tool_middleware_fn(
        move |call, ctx, next| {
            let c = counter.clone();
            Box::pin(async move {
    ...
    let config = LoopConfig {
        system_prompt: agent_types::SystemPrompt::Text("Echo with logging.".into()),
        ..Default::default()
    };
...
async fn agent_respects_max_turns() {
    ...
    let config = LoopConfig {
        system_prompt: agent_types::SystemPrompt::Text("Keep echoing.".into()),
        max_turns: Some(2),
        ..Default::default()
    };
...
fn anthropic_module_accessible() {
    let _provider = agent_blocks::anthropic::Anthropic::new("test-key");
}
