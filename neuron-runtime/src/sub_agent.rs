...
use neuron_loop::{AgentLoop, AgentResult, LoopConfig};
...
impl SubAgentManager {
    ...
    pub async fn spawn<P: Provider, C: ContextStrategy>(
        ...
        let mut neuron_loop = AgentLoop::new(provider, filtered_tools, context, loop_config);
        let result = neuron_loop.run(user_message, tool_ctx).await?;
        Ok(result)
    }
